...
VectorR3 myLightPositions[3] = {
	VectorR3(-5.0, 9.0, 0.0),
	VectorR3(2.5, 9.0, 2.5*sqrt(3)),
	VectorR3(2.5, 9.0, -2.5*sqrt(3)),
};
...
{
    globalPhongData.NumLights = 6;     // Should be enough lights for most 155A programming projects
...
void MySetupLights()
{
    ...
    // First light (light #0).
    myLights[0].AmbientColor.Set(0.13, 0.4, 0.4);   // Cyan
    myLights[0].DiffuseColor.Set(0.2, 0.7, 0.7);    // Cyan
    myLights[0].SpecularColor.Set(0.7, 0.9, 0.9);   // Bright cyan
    myLights[0].IsEnabled = true;                   // BE SURE TO ENABLE YOUR LIGHTS

    // Second light (light #1)
    myLights[1].AmbientColor.Set(0.4, 0.4, 0.13);   // Yellow
    myLights[1].DiffuseColor.Set(0.7, 0.7, 0.2);    // Yellow
    myLights[1].SpecularColor.Set(0.9, 0.9, 0.7);   // Bright yellow
    myLights[1].IsEnabled = true;

    // Third light (light #2)
    myLights[2].AmbientColor.Set(0.4, 0.13, 0.4);   // Magenta
    myLights[2].DiffuseColor.Set(0.7, 0.2, 0.7);    // Magenta
    myLights[2].SpecularColor.Set(0.9, 0.7, 0.9);   // Bright magenta
    myLights[2].IsEnabled = true;

	myLights[3].DiffuseColor.Set(0.6, 0.6, 0.6);      // Very light gray
	myLights[3].SpotCosCutoff = 0.9f;
	...
	myLights[4].SpotCosCutoff = 0.9f;
	myLights[4].IsSpotLight = true;
	myLights[4].SpotExponent = 1.0f;
	myLights[4].IsEnabled = true;                   // BE SURE TO ENABLE YOUR LIGHTS

	myLights[5].DiffuseColor.Set(0.6, 0.6, 0.6);      // Very light gray
	myLights[5].SpotCosCutoff = 0.9f;
	myLights[5].IsSpotLight = true;
	myLights[5].SpotExponent = 1.0f;
	myLights[5].IsEnabled = true;                   // BE SURE TO ENABLE YOUR LIGHTS
...
void LoadAllLights() 
{
	...
	myLights[3].SetPosition(viewMatrix, VectorR3(1.0, 9.0, 0.0));
	myLights[3].SetSpotlightDirection(viewMatrix, VectorR3(-0.1, -1.0, 0.0));
	myLights[3].LoadIntoShaders(3);

	myLights[4].SetPosition(viewMatrix, VectorR3(-0.5, 9.0, 0.5*sqrt(3)));
	myLights[4].SetSpotlightDirection(viewMatrix, VectorR3(0.05, -1.0, -0.05*sqrt(3)));
	myLights[4].LoadIntoShaders(4);

	myLights[5].SetPosition(viewMatrix, VectorR3(-0.5, 9.0, -0.5*sqrt(3)));
	myLights[5].SetSpotlightDirection(viewMatrix, VectorR3(0.05, -1.0, 0.05*sqrt(3)));
